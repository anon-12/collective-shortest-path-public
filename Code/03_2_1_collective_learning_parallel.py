'''
3.2.1 Collective Learning Parallel

Parallelized Implementation of Collective Search for Minimal Arrival Time
'''
...
def process_queries(start_row,end_row,process_id,queries_for_la,intersecting_paths,queries,node_to_node_best_paths,G,elm,load_aware_a_star):

    #Create Query Set
    queries_to_process = queries_for_la.iloc[start_row:end_row]
    
    #Create Results Set
    
    process_results = pd.DataFrame(index = queries_to_process.index,columns = ['Query','Q ID','Q Best Path','Visited','Path','Congested Edges','Load Aware Arrival Time'])
    
    #Process Queries
    for index_la, row_la in queries_to_process.iterrows():
        if index_la in intersecting_paths:
            ...
            process_results.loc[index_la]['Query'] = q_to_check
            process_results.loc[index_la]['Q ID'] = q_to_check_id
            process_results.loc[index_la]['Q Best Path'] = q_to_check_best_path
            
            visited,path,congested_flow_edges = load_aware_a_star(q_to_check,G,elm,best_path,n2n_best_lengths)
            
            process_results.loc[index_la]['Visited'] = visited
            process_results.loc[index_la]['Path'] = path
            process_results.loc[index_la]['Congested Edges'] = congested_flow_edges
            process_results.loc[index_la]['Load Aware Arrival Time'] = round(row_la['t'] + visited[row_la['to']],4)
    ...
    if congestion_encountered == False:
        ...
        if re_check_q == False:
            ...
            for p_id in intersecting_paths:
                ...
                la_results = la_results.drop(p_id)
        ...
        if queries_for_la.shape[0] > 0:
        
            #Parallel Process Queries Across Available CPUs
            
            #Get number of queries to process
            num_queries = queries_for_la.shape[0]
            #Number of queries per process
            step_size = get_step_size(num_queries, processes)
            
            # Split up trips for arguments
            args = []
            process_id = 0
            for i in range(processes):
                ...
                arg = (
                    offset, 
                    min(offset+step_size, num_queries),
                    process_id,
                    queries_for_la,
                    intersecting_paths,
                    queries,
                    node_to_node_best_paths,
                    G,
                    elm,
                    load_aware_a_star
                    )
            ...
            result = pool.starmap_async(process_queries, args).get()
            pool.close()
            
            results_for_joining = []
            
            for i in range(0,processes):
                results_for_joining.append(result[i])
                
            processed_queries_results = pd.concat(results_for_joining)
            
            if re_check_q == False:
                la_results = processed_queries_results.copy()
            else:
                la_results = pd.concat([processed_queries_results,la_results])
            
            #Add Results To EPM
            
            print('Adding to EPM')
            print('Size of results set : ' + str(processed_queries_results.shape[0]))
            
            for index,row in processed_queries_results.iterrows():
                q_epm = update_path_matrix(row['Query'],row['Path'],q_epm,row['Q ID'],row['Visited'])
